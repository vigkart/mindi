...
for algo_num in algo_list[1:]:
    ...
    X_init = X_init.join(features_df, how='inner')
    y_init = y_init.join(pnl, how='inner')
...
X_train, X_test, y_train, y_test = train_test_split(X_init, y_classified, test_size=0.2, random_state=42)
...
X_test_scaled = scaler.transform(X_test) # Scaling test set using scaler from training set
...
scaler_path = os.path.join(classification_data_dir, 'blackbox_classification_scaler.joblib')
