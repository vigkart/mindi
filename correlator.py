...
for col in df.columns[2:]:
    ...
    for space in range(col_spaces):
        col_name += " "
    print(
        f"stat: {col_name} | spearman: {round(spearman,5) } & pval {round(spval, 5)}, pearson: {round(pearson, 5)} & pval: {round(ppval, 5)}"
    )
    ...
    for percentile in percentiles:
        ...
        tile_data[col][percentiles[percentiles == percentile].index[0]] = tile[
            f"{algo_num}_pnl"
        ].mean()
